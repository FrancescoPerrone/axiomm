...
> is the only end-user-runnable tool today, so this README documents
> it. Work has begun on the **analysis suite** (`axiomm.analysis.*`,
> stage two) — decomposition, clustering, mineral labelling, reports —
> but its tools are not yet user-runnable; see the
> [Roadmap](https://github.com/FrancescoPerrone/axiomm/wiki/Roadmap)
> for stage-two status. The README broadens as each analysis tool
> becomes usable.
...
| Analysis suite | `axiomm.analysis.*`  | 🚧 Stage two, in progress. Shared foundations (S0) landed — error hierarchy, result-payload bases, backend registry, reference-library scaffolding. `decomposition` (PCA) is next; `clustering`, `mineralogy`, `reporting`, and a composable `axiomm.pipeline` follow. **Not yet user-runnable** — see the [Roadmap](https://github.com/FrancescoPerrone/axiomm/wiki/Roadmap). |
