...
> **Spec status (brought forward 2026-06-14).** Phases 0 → 4 of the
> chunk plan are implemented. Where the original spec named a single
> combined `XRMMapH5Config` dataclass, the implementation now uses the
> `HDF5MapSchema` (paths) + `XRMMapH5Calibration` (scientific values)
> split introduced in Phase 4 / Chunk 17, with the historic scientific
> constants extracted into the named preset
> `XRMMAP_LEGACY_APS_13_ID_E_PRESET_V1`. Every reader walks a
> resolution ladder (`source_metadata` → `user_config` →
> `legacy_preset` → `inferred` → `unknown`) gated by
> `ConversionMode` (default `GENERIC` since Phase 4 / Chunk 18). The
> sections below have been brought forward to that surface where the
> original phrasing would have been actively misleading; sections
> that still read accurately under the original wording have been
> left as-is. The chunk plan + per-phase status is the authoritative
> "what landed when" — see `docs/dev/STATE.md`.
...
The reader must not hard-code all scientific assumptions irreversibly. Phase 4 / Chunk 17 split the configurability surface in two: a **schema** (HDF5 paths) and a **calibration** (scientific values), mirroring the schema/calibration split that the generic HDF5 reader already used since Chunk 14. The split lets a single schema serve multiple instrument generations whose calibration constants differ.
...
# Built-in: the canonical XRM-Map / Larch layout.
XRMMAP_H5_SCHEMA = HDF5MapSchema(
...
class XRMMapH5Calibration:
    # Every field defaults to None so the resolution ladder can
    # distinguish "user supplied a value" from "use the preset".
...
The reader's constructor is kw-only and takes the two dataclasses plus a `ConversionMode`:

```python
from axiomm.io.converters import (
    ConversionMode, XRMMAP_H5_SCHEMA,
    XRMMapH5Calibration, XRMMapH5Reader,
)

reader = XRMMapH5Reader(
    schema=XRMMAP_H5_SCHEMA,                          # or omit for the default
    calibration=XRMMapH5Calibration(energy_scale=0.01),
    mode=ConversionMode.GENERIC,                      # default since Chunk 18
)
payload = reader.read("A21_054_map.h5")
```

When the calibration field is `None`, the reader's resolution ladder consults the active named preset (`XRMMAP_LEGACY_APS_13_ID_E_PRESET_V1` for the AXIOMM-inherited APS 13-ID-E XRM-Map dataset). See §15 *Scientific metadata policy* for the full ladder + provenance semantics.
...
The registry uses lazy `"module:attr"` factories so importing the
registry doesn't pull in optional runtime dependencies. Plugin
discovery via Python entry points landed in Phase 3 / Chunk 13:
third-party packages declare their readers/writers under the
`axiomm.readers` / `axiomm.writers` entry-point groups and AXIOMM
picks them up automatically at registry-load time.
...
class CalibrationUnresolvedError(AxiommConverterError):
    # Phase 4 / Chunk 16. Raised by readers in ConversionMode.STRICT
    # when a required calibration value cannot be resolved from
    # explicit user config or source metadata.
    ...
```
...
These three buckets are surfaced verbatim on every payload as `signal.metadata.AXIOMM.provenance_classification` and on the manifest sidecar.

### 15.1 Per-value calibration provenance (Phase 4)

Phase 4 added a second, **per-value** provenance axis on top of the three-bucket classification. Every scientific calibration scalar (energy scale, navigation pixel scale, ROI-limit unit interpretation, …) is resolved through a named ladder and stamped with its source:

```python
class CalibrationSource(str, Enum):
    SOURCE_METADATA = "source_metadata"   # read from the source file
    USER_CONFIG     = "user_config"       # explicit XRMMapH5Calibration field
    LEGACY_PRESET   = "legacy_preset"     # named preset (e.g. XRMMAP_LEGACY_APS_13_ID_E_PRESET_V1)
    INFERRED        = "inferred"          # heuristic; always carries a diagnostic
    UNKNOWN         = "unknown"           # unresolved; STRICT mode raises


class ConversionMode(str, Enum):
    LEGACY     = "legacy"      # preset fallback at info severity
    GENERIC    = "generic"     # preset fallback at warning severity (DEFAULT)
    STRICT     = "strict"      # no preset fallback; UNKNOWN → raise
    DIAGNOSTIC = "diagnostic"  # dry-run reporting
```

Each resolved value is carried on the payload as a frozen `ResolvedValue(value, source, note)` triple. The reader emits one diagnostic per source-grouping (`calibration_resolved_from_user_config` / `_from_metadata` / `_from_preset` / `calibration_inferred`).

### 15.2 Mapping the three buckets to the per-value ladder

The two systems coexist. The per-value `CalibrationSource` is finer-grained; the three-bucket classification at the AXIOMM-namespace level is built by aggregating across all fields a reader populated:

- `SOURCE_METADATA` per-value → `observed` bucket.
- `USER_CONFIG` / `LEGACY_PRESET` / `INFERRED` per-value → `assumed` bucket.
- Inferred axis sizes from `data.shape` → `inferred` bucket.

### 15.3 AXIOMM namespace layout
...
payload.metadata["AXIOMM"] = {
    "converter": {
        "reader": "xrmmap_h5",
        "reader_version": "...",
        "config": {                       # the {schema, calibration, mode} bundle
            "schema": {...},
            "calibration": {...},
            "mode": "generic",
        },
...
The manifest sidecar (schema v2 since Chunk 10) mirrors `signal.metadata.AXIOMM` byte-for-byte under an `axiomm_metadata` subkey.
...
The migrated XRM map reader reproduces these structural defaults
unless overridden via the schema:
...
The legacy scientific constants (per-channel energy width, ROI-limit
unit interpretation, scan-field fallback) are **not** universal
defaults. They live in the named preset
`XRMMAP_LEGACY_APS_13_ID_E_PRESET_V1` and are consulted by the
reader's resolution ladder only when the active `ConversionMode`
permits preset fallback:

```python
XRMMAP_LEGACY_APS_13_ID_E_PRESET_V1 = XRMMapH5Calibration(
    energy_scale=40.96 / 4096,        # ≈ 0.01 keV/channel
    roi_limit_units="channel_index",  # integer ROI limits are channel indices
    legacy_field_width_um=500.0,      # scan-field extent, not beam size
    roi_variant_index=0,
)
```

The 2026-06-12 metadata audit on the inherited
`~/Desktop/research/melts/data/Maps-HDF5/` dataset confirmed these
values against the source HDF5 paths
(`/xrmmap/config/mca_calib/slope = 0.01`,
`/xrmmap/mcasum/energy[-1] = 40.95`,
`/xrmmap/config/rois/limits` storing channel-index integers,
scan-field width derived from `/xrmmap/config/scan/start1` /
`stop1`). They are however **dataset-specific**, not universal —
the original spec's open question ("must be justified, parameterised,
or extracted from metadata before public release") was resolved
architecturally rather than by declaring fixed values: the resolution
ladder + the named preset + the four conversion modes together let
public users supply their own calibration explicitly (in any mode) or
opt into the legacy preset for inherited files (`LEGACY` / `GENERIC` /
`DIAGNOSTIC` modes). Source-metadata extraction of the values from
the audit-confirmed paths is the natural next-extension item; see
`docs/dev/STATE.md` *Next chunk*.
...
### Phase 1: CLI and notebook usability — partial

Done:

- Replace print statements with logging.
- Add manifest writing (schema v2 since Chunk 10).

Outstanding (blocked on the UX-layout decision — see
`docs/dev/STATE.md` *Open decisions*):

- Implement `axiomm-convert` CLI.
- Add `convert_many` batch helper.
- Add optional Tkinter helper.
- Add notebook examples.

### Phase 2: configurability and validation ✅ done

Tasks (all completed; the original "Add `XRMMapH5Config`" item
landed as the combined dataclass and was later split in Phase 4 /
Chunk 17 into `HDF5MapSchema` + `XRMMapH5Calibration`):

1. Add the configurability surface (schema + calibration).
2. Parameterise energy scale, ROI-limit interpretation, spatial
   geometry.
3. Add metadata provenance classification (three buckets +
   per-value ladder).
4. Add stricter axis validation.
5. Add end-to-end tests against a real-shape regression fixture
   (`tests/io/converters/test_realistic_xrmmap_regression.py`).

### Phase 3: extensibility ✅ done

Tasks (Chunks 12–14; "additional output formats" remains in
*Backlog* until a concrete scientific need surfaces):

1. Add reader registry.
2. Add writer registry.
3. Add generic HDF5 schema-driven reader prototype
   (`GenericHDF5MapReader` + `HDF5MapSchema`).
4. Add plugin discovery via Python entry points
   (`axiomm.readers` / `axiomm.writers`).
5. Add additional output formats only when scientifically justified
   (parked).

### Phase 4: calibration provenance & resolution ✅ done

Driven by the 2026-06-12 geology-team policy reply on the three
legacy scientific constants. Tasks (Chunks 15–19):

1. Calibration provenance primitives — `CalibrationSource`,
   `ConversionMode`, `ResolvedValue`. Payload + metadata + manifest
   plumbing.
2. Resolution-ladder helpers (`resolve_energy_scale`,
   `resolve_roi_limit_interpretation`,
   `resolve_navigation_scale_calibration`,
   `compute_roi_scale_from_units`,
   `raise_if_strict_unresolved`) + mode plumbing in both readers.
3. Legacy preset extraction
   (`XRMMAP_LEGACY_APS_13_ID_E_PRESET_V1`) +
   `XRMMapH5Config` split into `HDF5MapSchema` +
   `XRMMapH5Calibration`.
4. Explicit-units `roi_limit_units` + explicit-geometry
   `field_width_um` / `field_height_um` / `pixel_size_um`;
   `HDF5MapConfig` → `HDF5MapCalibration` rename;
   `fallback_field_width_um` → `legacy_field_width_um` rename;
   **default mode flipped to `GENERIC`**.
5. Documentation closure (user-guide rewrite, wiki sweep,
   STATE.md / HANDOFF.md updates).
